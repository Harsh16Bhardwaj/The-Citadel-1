...
window.addEventListener('scroll', function() {
  ...
  if (stopSectionTop <= stickyDivHeight + 20) {
    ...
    stickyDiv.style.top = (window.scrollY + stopSectionTop - stickyDivHeight - 0) + '500px';
  } else {
...
document.getElementById('bootstrap-css-link').addEventListener('click', function() {
  toggle.classList.toggle("is-clicked");
  const link = '<link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-QWTKZyjpPEjISv5WaRU9OFeRpok6YctnYmDr5pNlyT2bRjXh0JMhjY6hW+ALEwIH" crossorigin="anonymous"></link>'; 
  navigator.clipboard.writeText(link).then(function() {
      console.log('Link copied to clipboard!');
  }, function(err) {
      console.error('Could not copy text: ', err);
...
document.getElementById('bootstrap-js-link').addEventListener('click', function() {
  const link = '<script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js" integrity="sha384-YvpcrYf0tY3lHB60NNkmXc5s9fDVZLESaAA55NDzOxhy9GkcIdslK1eN7N6jIeHz" crossorigin="anonymous"></script>'; 
  navigator.clipboard.writeText(link).then(function() {
      console.log('Link copied to clipboard!');
  }, function(err) {
      console.error('Could not copy text: ', err);
...
document.getElementById('pico-link').addEventListener('click', function() {
  const link = '<link rel="stylesheet"href="https://cdn.jsdelivr.net/npm/@picocss/pico@2/css/pico.min.css"/>'; 
  navigator.clipboard.writeText(link).then(function() {
      console.log('Link copied to clipboard!');
  }, function(err) {
      console.error('Could not copy text: ', err);
